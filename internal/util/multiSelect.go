...
func (s *MultiSelect) innerRun(cursorPos, scroll int, top rune) ([]int, error) {
	...
	cur := promptui.NewCursor("", s.Pointer, false)
	...
	c.SetListener(func(line []rune, pos int, key rune) ([]rune, int, bool) {
		...
		case key == promptui.KeyBackspace || key == promptui.KeyCtrlH:
		...
		if searchMode {
			header := promptui.SearchPrompt + cur.Format()
...
func (s *MultiSelect) prepareTemplates() error {
	...
	if tpls.FuncMap == nil {
		tpls.FuncMap = promptui.FuncMap
	}

	if tpls.Label == "" {
		tpls.Label = fmt.Sprintf("%s {{.}}: ", promptui.IconInitial)
	}
	...
	if tpls.Active == "" {
		tpls.Active = fmt.Sprintf("%s{{.}}", promptui.IconSelect)
	}
	...
	if tpls.Selected == "" {
		tpls.Selected = fmt.Sprintf(` {{ "%s" | green }} {{.}}`, promptui.IconGood)
	}
	...
	if tpls.Unselected == "" {
		tpls.Unselected = fmt.Sprintf(` {{ "%s" | red }} {{.}}`, promptui.IconBad)
	}
...
func (s *MultiSelect) setKeys() {
	...
	s.Keys = &MultiSelectKeys{
		...
		Toggle:   promptui.Key{Code: ' ', Display: "SPACE"},
